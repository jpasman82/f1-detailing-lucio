...
const AppContent = () => {
  ...
  const stats = useMemo(() => {
    ...
    const currentDayOfWeek = startOfCurrentWeek.getDay() || 7; 
    startOfCurrentWeek.setDate(startOfCurrentWeek.getDate() - (currentDayOfWeek - 1)); // Fija al Lunes
    ...
    // --- CÁLCULO ESTRICTO DE PROYECCIÓN (Semanas y Días faltantes) ---
    // 1. Ganancia ideal de 1 semana cumpliendo la meta al 100% con autos Full
    const profitPerFullCarUSD = config.priceFull / exRate;
    const idealWeeklyProfitUSD = profitPerFullCarUSD * weeklyGoal;

    let etaText = "";
    if (remainingUSD <= 0) {
      etaText = "¡OBJETIVO CUMPLIDO!";
    } else if (idealWeeklyProfitUSD > 0) {
      // 2. Tiempo restante exacto (en días)
      const totalDaysLeft = (remainingUSD / idealWeeklyProfitUSD) * 7;
      const weeksLeft = Math.floor(totalDaysLeft / 7);
      const daysLeft = Math.round(totalDaysLeft % 7);
      
      let timeParts = [];
      if (weeksLeft > 0) timeParts.push(`${weeksLeft} sem${weeksLeft !== 1 ? 's' : ''}`);
      if (daysLeft > 0) timeParts.push(`${daysLeft} día${daysLeft !== 1 ? 's' : ''}`);
      if (timeParts.length === 0) timeParts.push('Menos de un día');
      
      etaText = `Faltan aprox. ${timeParts.join(' y ')}`;
    } else {
      etaText = "Meta en rojo, revisa números";
    }
      ...
      <main className="p-4 space-y-6 max-w-md mx-auto">
        {activeTab === 'dashboard' ? (
          <>
            ...
            {/* Cajas de Efectivo y Gastos */}
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-slate-900 p-5 rounded-[2rem] border border-slate-800 flex flex-col justify-center relative overflow-hidden">
                <div className="absolute top-0 right-0 w-16 h-16 bg-blue-500/10 rounded-full -mr-8 -mt-8 blur-xl"></div>
                <p className="text-[9px] font-black text-slate-500 mb-1 uppercase tracking-widest relative z-10">Efectivo (UYU)</p>
                <p className="text-2xl font-black italic text-blue-400 tracking-tighter relative z-10">${Math.round(stats.totalSales + stats.totalTips).toLocaleString()}</p>
                <p className="text-[9px] text-slate-600 mt-1 uppercase tracking-widest font-bold relative z-10">Caja base + Propinas</p>
              </div>
              <div className="bg-slate-900 p-5 rounded-[2rem] border border-slate-800 flex flex-col justify-center relative overflow-hidden">
                <div className="absolute top-0 right-0 w-16 h-16 bg-red-500/10 rounded-full -mr-8 -mt-8 blur-xl"></div>
                <p className="text-[9px] font-black text-slate-500 mb-1 uppercase tracking-widest relative z-10">Gastos (UYU)</p>
                <p className="text-2xl font-black italic text-red-500 tracking-tighter relative z-10">-${Math.round(stats.totalExpenses).toLocaleString()}</p>
                <p className="text-[9px] text-slate-600 mt-1 uppercase tracking-widest font-bold relative z-10">Insumos de Pit Stop</p>
              </div>
            </div>

            {/* Botones de Acción */}
            <div className="grid grid-cols-2 gap-5">
              <button onClick={handleOpenWashModal} className="bg-red-700 p-8 rounded-[3rem] border-b-8 border-black active:translate-y-2 active:border-b-0 transition-all flex flex-col items-center gap-4 group shadow-xl">
                <Plus className="text-white group-active:scale-90 transition-transform" size={32} />
              ...
              <button onClick={() => setShowExpenseModal(true)} className="bg-slate-900 p-8 rounded-[3rem] border-b-8 border-black active:translate-y-2 active:border-b-0 transition-all flex flex-col items-center gap-4 group shadow-xl">
                <Receipt className="text-slate-400 group-active:scale-90 transition-transform" size={32} />
